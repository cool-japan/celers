...
impl TaskHookRegistry {
    ...
    pub async fn execute_before_enqueue(
        ...
        let hooks_count = {
            let guard = self.enqueue_hooks.read().unwrap();
            guard.len()
        };
    ...
    pub async fn execute_before_dequeue(&self, ctx: &HookContext) -> HookResult<()> {
        let hooks_count = {
            let guard = self.dequeue_hooks.read().unwrap();
            guard.len()
        };
    ...
    pub async fn execute_on_completion(
        ...
        let hooks_count = {
            let guard = self.completion_hooks.read().unwrap();
            guard.len()
        };
