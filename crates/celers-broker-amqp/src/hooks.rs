...
impl AmqpHookRegistry {
    ...
    pub async fn execute_before_publish(
        ...
        let hooks_count = {
            let guard = self.publish_hooks.read().unwrap();
            guard.len()
        };
    ...
    pub async fn execute_before_consume(&self, ctx: &HookContext) -> HookResult<()> {
        let hooks_count = {
            let guard = self.consume_hooks.read().unwrap();
            guard.len()
        };
    ...
    pub async fn execute_on_acknowledgment(
        ...
        let hooks_count = {
            let guard = self.ack_hooks.read().unwrap();
            guard.len()
        };
    ...
    pub async fn execute_on_error(&self, error: &str, ctx: &HookContext) -> HookResult<()> {
        let hooks_count = {
            let guard = self.error_hooks.read().unwrap();
            guard.len()
        };
