...
async fn export_all_tasks(broker: &MysqlBroker) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Save to file
    let backup_file = "/tmp/celers_tasks_backup.json";
    fs::write(backup_file, &json_data)?;

    println!("Exported all tasks to: {}", backup_file);
...
async fn export_tasks_by_state(
    ...
    // Save to file
    let backup_file = format!("/tmp/celers_tasks_{}.json", state);
    fs::write(&backup_file, &json_data)?;

    println!("Exported {} tasks to: {}", state, backup_file);
...
async fn import_tasks_example(broker: &MysqlBroker) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Importing tasks from: {}", backup_file);
...
async fn export_dlq_example(broker: &MysqlBroker) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dlq_file = "/tmp/celers_dlq_export.json";
    fs::write(dlq_file, &json_data)?;

    println!("Exported DLQ to: {}", dlq_file);
    println!("File size: {} bytes", json_data.len());
...
async fn backup_restore_workflow(broker: &MysqlBroker) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let backup_dir = "/tmp/celers_backup";
    fs::create_dir_all(backup_dir)?;

    // Export all tasks
    let all_tasks = broker.export_tasks(None, None).await?;
    fs::write(format!("{}/all_tasks.json", backup_dir), &all_tasks)?;

    // Export by state
    let pending_tasks = broker
        .export_tasks(Some(DbTaskState::Pending), None)
        .await?;
    fs::write(format!("{}/pending_tasks.json", backup_dir), &pending_tasks)?;

    // Export DLQ
    let dlq_data = broker.export_dlq(None).await?;
    fs::write(format!("{}/dlq.json", backup_dir), &dlq_data)?;
    ...
    let all_tasks_json = fs::read_to_string(format!("{}/all_tasks.json", backup_dir))?;
