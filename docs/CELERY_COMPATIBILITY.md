...
| PostgreSQL | ⛔ | ✅ | n/a | CeleRS-specific; Celery has no PostgreSQL broker. **Live-verified against a real PostgreSQL 16, 230/230** (`tests_pg.rs` + `tests_pg_binds.rs`, gated on `CELERS_TEST_POSTGRES_URL`) — this release fixed a bare-`$n` UUID/JSONB bind defect that made the broker non-functional against a live server; see [CHANGELOG.md](../CHANGELOG.md)'s 0.3.1 "Fixed" section |
| MySQL | ⛔ | ✅ | n/a | CeleRS-specific. **Live-verified against a real MySQL 8, 217/217** (gated on `CELERS_TEST_MYSQL_URL`/`MYSQL_URL`), under both `nextest`'s default parallel execution and `--test-threads=1` — this release fixed missing deadlock retry, an uncreatable `move_to_dlq`, and a queue-scoping leak across the diagnostics methods; see CHANGELOG.md. **Caveat:** pointing this broker and `celers-backend-db`'s MySQL result backend at the *same* database corrupts one of the two — [TODO.md → Known gaps #16](../TODO.md#known-gaps--the-roadmap-after-031) |
...
| PostgreSQL | ✅ (SQLAlchemy) | ✅ | 🔴 Not interoperable | Different schema entirely. **Live-verified against a real PostgreSQL 16** — this release fixed a bare-`$n` JSONB bind defect (`result_data`/`extra`/`task_ids`) that made storing a result with a payload fail outright against a live server; see [CHANGELOG.md](../CHANGELOG.md)'s 0.3.1 "Fixed" section. `celers-backend-db`'s full suite (`DATABASE_URL` set, `MYSQL_URL` pointed at any database `celers-broker-sql` has not migrated) is **150/150** |
| MySQL | ✅ (SQLAlchemy) | ✅ | 🔴 Not interoperable | Different schema entirely. **Live-verified against a real MySQL 8** — this release fixed a `migrate()` that failed outright on every MySQL database (an uncreatable `DELIMITER //` stored-procedure body) and a `SUM`-over-empty-result crash in `task_stats`. **Caveat:** `MYSQL_URL` pointed at the *same* database as `celers-broker-sql`'s MySQL broker collides on the shared table name `celers_task_results` and 7 tests fail; point it at a database `celers-broker-sql` has not migrated for a clean **150/150** — [TODO.md → Known gaps #16](../TODO.md#known-gaps--the-roadmap-after-031) |
