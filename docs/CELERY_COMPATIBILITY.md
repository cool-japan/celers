...
| PostgreSQL | ⛔ | ✅ | n/a | CeleRS-specific; Celery has no PostgreSQL broker |
| MySQL | ⛔ | ✅ | n/a | CeleRS-specific |
| In-memory (dev) | ⛔ | ✅ | n/a | `celers_core::InMemoryBroker`, for local development and tests |
| RabbitMQ (AMQP) | ✅ | ⛔ **transport only** | 🔴 Not interoperable | `AmqpBroker` implements the `celers_kombu` transport traits, **not** `celers_core::Broker`, so a `celers_worker::Worker` cannot consume from it today (an adapter is the missing piece — see the note in `celers-broker-amqp`'s `lib.rs`). It publishes a JSON-serialized `celers_protocol::Message` envelope as the AMQP body rather than mapping the v2 headers onto AMQP basic-properties headers the way kombu's AMQP transport does; untested against a real Celery AMQP consumer |
| Amazon SQS | ✅ | ⛔ **transport only** | 🔴 Not interoperable | Same shape as AMQP: `SqsBroker` implements the `celers_kombu` traits only. SQS also has no message-level priority, so priorities mean separate queues. [ADR-005](adr/005-sqs-celery-compatibility.md) sketches a Kombu-compatible SQS design but is **Proposed and unimplemented** — read it as a plan, not a description |
...
| PostgreSQL / MySQL | ✅ (SQLAlchemy) | ✅ | 🔴 Not interoperable | Different schema entirely |
...
| Solar | ✅ | ✅ | Behind `celers-beat/solar`. Resolves via `sunrise`'s `SolarDay::event_time` (absolute `DateTime<Utc>`). Supports sunrise/sunset, civil/nautical/astronomical twilight as true elevation solves, and golden hour as an offset approximation; handles polar day/night by skipping dates with no such event |
